(function() {
  var ansi, im, interactiveMode, mp, parseArgs, parsePort, parseType, printUsageAndExit, usage, validatePort, validateType;
  ...
  ['pull', 'sub'].forEach(function(name) {
    return im[name] = function(portNumber, messagePort) {
  ...
  ['push', 'pub'].forEach(function(name) {
    return im[name] = function(portNumber, messagePort, getLine) {
      var getAndSendMsg;
      im.info("started " + (im.longName(name)) + " socket on port " + portNumber);
